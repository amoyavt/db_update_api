...
using Edge.Service.Workers;
using Edge.Service.Configuration;
using Edge.Service.Extensions;
using Serilog;
...
builder.Services.AddHealthChecks()
    .AddDbContext<EdgeDbContext>("database")
    .AddHttpClient("central-api", client => 
    {
        client.BaseAddress = new Uri(builder.Configuration["CentralApi:BaseUrl"] ?? "http://host.docker.internal:8080");
...
try
{
    Log.Information("Starting Edge Service");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Edge Service terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
