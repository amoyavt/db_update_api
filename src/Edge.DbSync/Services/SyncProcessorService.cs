...
public class SyncProcessorService : ISyncProcessorService
{
    ...
    {
        var tables = new[] { "\"Schedules\"", "\"Devices\"", "\"Areas\"", "\"Users\"", "\"Groups\"", "\"Locations\"", "\"Companies\"" };
    ...
    private async Task LoadDataAsync(Dictionary<string, object[]> data)
    {
        ...
        await LoadSchedulesAsync(data["schedules"]);
    }
    ...
    private async Task LoadSchedulesAsync(object[] data)
    {
        var schedules = JsonSerializer.Deserialize<ScheduleDto[]>(JsonSerializer.Serialize(data), _jsonOptions);
        ...
        _context.Schedules.AddRange(entities);
        await _context.SaveChangesAsync();
        _logger.LogDebug("Loaded {Count} schedules", schedules.Length);
    }
